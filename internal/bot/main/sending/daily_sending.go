...
func (sm *SendingManager) processDailySending(t time.Time) {
	...
	chats, err := model.GetChatsByDailySendingTime(sm.services.Repository.DB, timeStr)
	if err != nil {
	...
	groupedChats := make(map[model.GroupName][]*model.Chat)
	for _, chat := range chats {
		if groupedChats[*chat.GroupName] == nil {
			groupedChats[*chat.GroupName] = []*model.Chat{}
		}
...
func (sm *SendingManager) sendDailyNotificationToGroups(groupedChats map[model.GroupName][]*model.Chat) ([]error, int) {
...
func (sm *SendingManager) sendWeekScheduleToGroup(groupName model.GroupName, chats []*model.Chat) ([]error, bool) {
	log.Trace().Any("group", groupName).Msg("Preparing schedule for group...")
	ctx := context.Background()
	var errors []error

	// Prepare schedule
	group, err := model.GetGroupByName(sm.services.Repository.DB, groupName)
	if err != nil {
