...
func (sm *SendingManager) RunChangeAlertNotifier(ctx context.Context) {
	...
	var changes = make(chan *model.ScheduleChange)
...
func (sm *SendingManager) sendChangeAlertForGroup(ctx context.Context, change *model.ScheduleChange) error {
	log.Trace().Any("config", change.New.Config).Msg("Sending change alert for group...")
	chats, err := sm.services.Chat.AllByWatchedGroup(change.New.Config.Group.GroupName)
	if err != nil {
		return err
	}
...
func (sm *SendingManager) runSchedulepdateMonitor(ctx context.Context, changes chan<- *model.ScheduleChange) {
	...
	for {
		...
		// Fetch groups
		groups, err := sm.services.Chat.WatchedGroups()
		if err != nil {
		...
		chatCount, err := sm.services.Chat.CountChangeAlertOn()
		if err != nil {
...
func (sm *SendingManager) fetchScheduleUpdates(
	groups []model.Group,
	changes chan<- *model.ScheduleChange,
	...
	elapsed := measureTime(func() {
		for _, group := range groups {
			...
			newRawSchedule, err := sm.services.ScheduleMan.UpdateCache(sm.services.Browser, conf)
			if err != nil {
			...
			if len(diffs) > 0 {
				...
				changesDetected++
				if chatsCount, err := sm.services.Chat.CountByGroup(group.GroupName); err != nil {
					chatsAffected++
				} else {
...
func (sm *SendingManager) sendChangeAlertReport(diffs []model.Diff, change *model.ScheduleChange) {
	var debugDiffs []model.Diff
	for _, d := range diffs {
		dd := d
		dd.OldDay = nil
		dd.NewDay = nil
